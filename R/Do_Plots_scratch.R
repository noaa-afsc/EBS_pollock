if (doplots) {
  ...
  df <- rbind(
    rbind(
      data.frame(Model = "This year", Year = M$SSB[, 1], SSB = M$SSB[, 2], lb = M$SSB[, 4], ub = M$SSB[, 5]),
    ...
    rbind(
      data.frame(Model = "Last year", Year = P$SSB[, 1], SSB = P$SSB[, 2], lb = P$SSB[, 4], ub = P$SSB[, 5]),
  ...
  p2 <- ggplot(dt, aes(x = ssb, y = rs, label = Year)) +
    geom_text() +
    geom_path(size = .5, alpha = .4) +
  ...
  df.g <- gather(df, age, F, 2:16, -Year)
  p1 <- df.g %>%
    mutate(age = as.numeric(age)) %>%
    filter(age < 11) %>%
    ggplot(aes(y = age, x = Year, fill = F)) +
    geom_tile() +
    .THEME +
    ylab("Age") +
...
p1 <- ggplot(dd.g, aes(x = Year, y = Biomass, color = Assessment)) +
  ...
  seldfm <- seldf %>%
    pivot_longer(cols = 3:17, names_to = "age", values_to = "Selectivity") %>%
    mutate(
      age = as.numeric(age),
  ...
  assdf <- seldfm %>%
    ...
    mutate(age = as.factor(age), Year = as.factor(Year))
  yrdf <- seldfm %>%
    filter(Year <= Assessment, Year > thisyr - 10) %>%
    mutate(age = as.factor(age), Year = as.factor(Year))
  p1 <- seldfm %>%
    filter(age < 9, Year > thisyr - 10) %>%
    group_by(Year, Assessment, case) %>%
  ...
  p3 <- tibble(regime = regime, Mean = M$regime, ub = Mean + 2 * M$regime.sd, lb = Mean - 2 * M$regime.sd) %>%
    ggplot(aes(x = regime, y = Mean, ymax = ub, ymin = lb)) +
  ...
  plot_coh <- function(dat = ret_df, coh = cohsub, logscale = FALSE, grid = T) {
    p1 <- dat |>
      ...
      ggplot(aes(x = Years_data, label = term_yr, y = est, color = run, ymin = lb, ymax = ub)) +
      geom_point(size = 1) +
  ...
  ggplot(
    ...
    aes(x = term_yr, y = est, color = run, ymin = lb, ymax = ub)
  ) +
    geom_point(size = 2) +
  ...
  p1 <- plot_R_rel(retouts, xlim = c(thisyr - 10, thisyr), legend = F, rel = FALSE, ylab = "Age 1 recruitment")
  p1
  p2 <- plot_R_rel(retouts, xlim = c(thisyr - 10, thisyr), ylim = c(0., 3.5), legend = FALSE, alpha = .2)
  p2 <- p2 + ylim(0, 4)
  p2
  p3 <- p1 / p2
  p3
  ggsave("doc/figs/mod_retroR.pdf", plot = p3, width = 7.2, height = 9.0, units = "in")
  p1 <- plot_R_rel(retouts, xlim = c(thisyr - 20, thisyr), legend = F, rel = FALSE)
  p1
  p2 <- plot_R_rel(retouts, xlim = c(thisyr - 20, thisyr), ylim = c(0., 3.5), legend = FALSE, alpha = .2)
  p2 <- p2 + ylim(0, 4)
  p2
  p3 <- p1 / p2
  ...
  cd_tab <- cd %>%
    filter(Area == "BSAI") %>%
    mutate(Subarea = ifelse(NMFS_area == 518, "Bogoslof",
      ifelse(NMFS_area < 520, "SE",
        ifelse(NMFS_area < 540, "NW", "Aleutians")
      )
    )) %>%
    group_by(Year, Subarea, ret_disc) %>%
    summarise(catch = sum(catch)) %>%
    pivot_wider(names_from = c(Subarea, ret_disc), values_from = catch) %>%
    flextable() %>%
    print(n = Inf)
  print(cd_tab)
  ...
  df.g
  p1 <- df.g %>% ggplot(aes(x = Year, y = val, col = Measure)) +
    .THEME +
    geom_line(size = 2) +
    ylim(0.25, 2.00) +
    ggtitle(paste0("Projected trend relative to ", thisyr, " given future catch=1,350 kt")) +
    ylab("Relative value")
  p1
  ...
  df.g %>%
    filter(Measure == "catch") %>%
    select(year, scen, val) %>%
    ggplot(aes(x = year, y = scen)) +
    .THEME +
    geom_density_2d()
  ...
  p1 <- ggplot(df, aes(x = Year, y = Biomass)) +
  ...
  p1 <- ggplot(df %>% filter(Area == "Both"), aes(x = Year, y = Biomass)) +
  ...
  srdf <- srdf %>%
    filter(draw %in% srsm) %>%
    mutate(stock = as.numeric(stock), recruits = as.numeric(recruits), draw = as.factor(draw)) # %>%
  srdf %>% ggplot(aes(x = stock, y = recruits, group = draw)) +
    geom_line(size = .1, alpha = .4, color = "coral") +
    theme_few() +
    xlim(c(0, 6000)) +
    ylim(c(0, 50000))
  srsm
...
radf <- radf %>%
  select(date, lat, lon, len, sex, wt, age) %>%
  mutate(date = lubridate::dmy(date), year = year(date), len = len / 10, sex = ifelse(sex == 1, "Male", ifelse(sex == 2, "Female", "U")))
radf %>%
  filter(year > 1999, wt > 0, age > 0, age < 15) %>%
  group_by(year, age) %>%
  summarise(wt = mean(wt)) %>%
  ggplot(aes(y = wt, x = age, col = as.factor(year))) +
  geom_point(alpha = .6) +
  theme_few()
radf %>%
  filter(year > 1999, wt > 0, age > 0, age < 15) %>%
  ...
  ggplot(aes(y = len, x = age, col = as.factor(year))) +
  geom_point(alpha = .6) +
  theme_few()
...
radf %>%
  filter(age > 0, age < 15) %>%
  group_by(year, age) %>%
  summarise(len = mean(len), wt = mean(wt)) %>%
  filter(age == 6) %>%
  ggplot(aes(y = len, x = year, )) +
  geom_line(size = 2) +
  theme_few()
...
radf %>%
  filter(year %in% c(2000, 2010), wt > 0, len < 70) %>%
  ggplot(aes(y = wt, x = len, color = as.factor(year))) +
  geom_point(alpha = .2) +
  theme_few()

radf %>%
  filter(year >= 2000, wt > 0, len < 70) %>%
  group_by(year, len) %>%
  summarise(wt = mean(wt)) %>%
  ggplot(aes(y = wt, x = len, color = as.factor(year))) +
  geom_line(size = .2) +
  theme_few()
radf %>%
  filter(year %in% c(2000, 2010, 2021), wt > 0, len < 70) %>%
  group_by(year, len) %>%
  summarise(wt = mean(wt)) %>%
  ggplot(aes(y = wt, x = len, color = as.factor(year))) +
  geom_line(size = 2) +
...
cadf <- adf %>%
  mutate(date = lubridate::ymd(paste(year, month, 01, sep = "-")), wt = wt * 1000) %>%
  select(date, lat, lon, sex, len, wt, age, year) %>%
  mutate(source = "Fishery") %>%
  rbind(radf %>% select(date, lat, lon, sex, len, wt, age, year) %>% mutate(source = "Survey"))
cadf %>%
  filter(year > 1999, wt > 0, age > 0, age < 15, month(date) > 5) %>%
  group_by(source, year, age) %>%
  summarise(wt = mean(wt), len = mean(len)) %>%
  filter(age == 6) %>%
  ggplot(aes(y = len, x = year, color = source)) +
  geom_line(size = 2) +
  theme_few()
cadf %>%
  filter(year > 1990, wt > 0, age > 0, age < 15, month(date) > 5) %>%
  group_by(source, year, age) %>%
  summarise(wt = mean(wt), len = mean(len)) %>%
  filter(age == 6) %>%
  ggplot(aes(y = wt, x = year, color = source)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  theme_few()
cadf %>%
  filter(year > 1990, wt > 0, age > 0, age < 15, month(date) > 5) %>%
  group_by(source, year, age) %>%
  summarise(n = n(), wt = mean(wt), len = mean(len)) %>%
  filter(age == 6) %>%
  ggplot(aes(y = n, x = year, color = source)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  theme_few()
...
if (do_data_plots) {
  ...
  wed %>%
    ...
    group_by(Year) %>%
    summarize(sum(Weight.Posted..Sum.)) %>%
    tail()
  df <- wed %>%
    filter(FMP.Area == "BSAI", Species.Group.Name == "Pollock") %>%
    transmute(Year,
      ret = Retained.Discarded, wed = WED..mmdd.,
  ...
  tsum <- (df) %>%
    group_by(ret, mgt, Year) %>%
    summarize(Catch = sum(Catch)) %>%
    pivot_wider(names_from = c(mgt, ret), values_from = Catch)
  p1 <- df %>%
    filter(!is.na(strata)) %>%
    mutate(strata = ifelse(strata == 1, "A-season", ifelse(strata == 2, "NW B-season", "SE B-season"))) %>%
  ...
  rd <- select(rd, Year, Season, Prod, t) %>%
    group_by(Year, Season, Prod) %>%
    summarize(t = sum(t))
  p1 <- rd %>%
    filter(Prod == "ROE") %>%
    ggplot(aes(x = Year, y = t, shape = Season, col = Season)) +
    geom_point(size = 3) +
    geom_line(size = 1.0) +
    theme_few(base_size = 14) +
  ...
  tot <- edt %>%
    ...
    mutate(season = ifelse(stratum == 1, "A", "B"), sex = ifelse(sex == 1, "Male", ifelse(sex == 2, "Female", "Total"))) %>%
    group_by(year, sex) %>%
  ...
  p1 <- edt %>%
    ...
    summarize(Catch = sum(value)) %>%
    rbind(tot) %>%
    ggplot(aes(x = year, y = Catch, color = season, shape = sex, linetype = season)) +
    theme_few(base_size = 12) +
    geom_point(size = 3.5) +
    geom_line(size = 1.2) +
    expand_limits(y = 0) +
  ...
  cdf_df <- df |>
    group_by(Year) |>
    summarise(Biomass = sum(Catch))
  left_join(edt_df, cdf_df) |> ggplot(aes(x = Number, y = Biomass, label = Year)) +
    theme_few() +
    geom_text(size = 3) +
    geom_smooth(method = lm)
  ...
  p1 <- ggplot(sdf, aes(x = age, y = as.factor(Year), height = n, fill = Model, type = Model, color = Model)) +
    geom_density_ridges(stat = "identity", scale = 4, alpha = .1) +
    theme_few() +
    xlim(c(1, lage)) +
    ylab("Year") +
    xlab("age")
  ...
  glimpse(catch)
    ...
    group_by(Year, Area, seas) %>%
    summarise(Catch = round(sum(Catch), 0))

  catchw <- catch %>%
    filter(Area != "Other") %>%
    pivot_wider(names_from = c(Area, seas), values_from = Catch)
  tail(catchw)
  ...
  p1 <- ggplot(td.g, aes(x = YEAR, y = temp, col = Source, shape = Source)) +
    geom_point(size = 2) +
    geom_line(size = 1.0) +
    .THEME +
    ylab("Degrees C") +
    ...
    geom_hline(yintercept = mst, linetype = "dashed") +
    geom_hline(yintercept = mbt, linetype = "dashed")
  p1
