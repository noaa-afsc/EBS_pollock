...
#| messages: FALSE
#| label: tbl-loo_fits
#| tbl-cap: "Goodness-of-fit measures to primary data for leave-one-out model sensitivity runs. RMSE=root-mean square log errors, SDNR=standard deviation of normalized residuals, Eff. N=effective sample size for composition data)." 
...
# Incorporating multi-species M-at-age and year estimates
...
#| fig.cap: "Comparison of recruitment estimates comparing 2024 assessment results with one where M-at-age and year is specified from CEATTLE."
  p2<-plot_recruitment(Mmat_lst, xlim = c(1964, 2024)); p2
```
...
#| messages: FALSE
#| label: tbl-Mmat_fits
#| tbl-cap: "Goodness-of-fit measures to primary data for model sensitivity to applying the matrix of M-at-age and year instead of constant M-at-age. RMSE=root-mean square log errors, SDNR=standard deviation of normalized residuals, Eff. N=effective sample size for composition data)." 
#(tab_fit(loo_lst[c(1:7)])) #|>
gt_table <- gt::gt(tab_fit(Mmat_lst[c(1:2)]))
numeric_cols <- 2:3 # Specify the numeric columns to format
target_rows <- c(8:22)
for (col in numeric_cols) {
  gt_table <- gt_table %>%
    fmt_number(
...
#| messages: FALSE
#| label: tbl-Mmat_ref
#| tbl-cap: "Estimated derived quantities and selected corresponding CVs for  sensitivity to natural mortality specification."
df <- NULL
mod_scen <- c(1:2)
for (ii in mod_scen) {
  x <- Mmat_lst[[ii]]
