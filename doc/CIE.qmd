...
method, the consistency was lower (@fig-consisFTN). The extent that this is a concern
is unknown.
...
availability among types of ageing is shown in @fig-dataExtent. These figures are 
intended to show what data are available for the model testing and 
the different ways of treating age determination error matrices.
...
The layout of model runs comparing the traditional microscope ages (TMA) with the
FT-NIRS ages is shown in @tbl-ftn_runs. The models are run with the FT-NIRS data with 
different age-error matrix specficiations. 
...
as well as a run that downweights all survey biomass indices. @tbl-loo_runs shows the combinations of data
types included for this analysis.
...
if (do_loo) {
  ...
  nextyr <- thisyr + 1
  dec_tab_ord <<- 1:8
  mod_names <- c("2024 accepted", "Drop BTS age comps", "Drop ATS age comps", "Drop survey indices")
...
#| label: tbl-loo_fits
#| tbl-cap: "Goodness-of-fit measures to primary data for different data and age-error applications. RMSE=root-mean square log errors, NLL=negative log-likelihood, SDNR=standard deviation of normalized residuals, Eff. N=effective sample size for composition data). See text for incremental model descriptions."
gt_table <- gt::gt(tab_fit(loo_lst[c(1:4)])) #|>
numeric_cols <- 2:5 # Specify the numeric columns to format
target_rows <- c(8:21)
...
knitr::include_graphics(here::here("doc", "figs", "loo_ssb.png"))
```
...
Comparing the SS3 output with three different configurations on selectivity constraint
shows xxx (@fig-ss3ts).
Comparing the SS3 output with last year's reference model shows that xxx (@fig-ss3tsref).
...
plotly::ggplotly(Plot_SSB(all_ts) + ggthemes::theme_few(base_size = 19))
...
plotly::ggplotly(Plot_SSB(all_ts) + ggthemes::theme_few(base_size = 19))
plotly::ggplotly(Plot_Rec(all_ts) + ggthemes::theme_few(base_size = 19))
# all_sel <- rbind(pm_run$sel,r1$sel,c2$sel)#,gp_run$sel)
# ss_sel  <- rbind(r1$sel,r2$sel,r3$sel)#,r4$sel)#,r5$sel)
```
## SAM
...
can be found 
[here](https://github.com/noaa-afsc/EBS_pollock/tree/main/runs/SAM/poll24 "SAM model runs" )). Comparing "selectivity" in the SAM model to the reference
model, we see broad similarities but slightly lower inter-annual variability in the SAM run (@fig-sam). The spawning biomass trend largely overlaps in the uncertainty (@fig-sam_ts).
...
plotly::ggplotly(Plot_SSB(rbind(sam_run$ts, pm_run$ts)) + ggthemes::theme_few(base_size = 19))
plotly::ggplotly(Plot_Rec(rbind(sam_run$ts, pm_run$ts)) + ggthemes::theme_few(base_size = 19))
# all_sel <- rbind(pm_run$sel,r1$sel,c2$sel)#,gp_run$sel)
# ss_sel  <- rbind(r1$sel,r2$sel,r3$sel)#,r4$sel)#,r5$sel)
```
...
lab and the University of Alaska, Fairbanks (@spock).  @fig-spock_sel shows the model results
comparing time-varying selectivity whereas @fig-spock_ts shows the model results time series. The difference in the age-1 recruits is likely due to the fact that the 
...
The estimates of abundance at age were bias-corrected with the same method as the abundance index. We implemented the same diagnostics to check convergence and model fit as those used for the abundance index. Results compared favorably among methods (@fig-sdm_age).
