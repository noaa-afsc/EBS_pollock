...
Get_EDF <- function(rundir, redo=FALSE) {
  ...
  h1 <- getADMBHess(filename=here(rundir, "h1.hes"))
  h0 <- getADMBHess(filename=here(rundir, "h0.hes"))
  ## Create a vector of indices where fixed-effect parameters----
  #pars <- read_fit( here("runs/lastyrdb/pm"))
  pars <- adnuts:::.read_mle_fit("pm", here(rundir))
  indx <- h1[1, ] ;
...
t2<- Get_EDF(rundir = here::here(rundir="runs/ftnir1/"), redo=TRUE)   |> mutate(Model="FTNIR1")  
t3<- Get_EDF(rundir = here::here(rundir="runs/ftnir2/"), redo=TRUE)   |> mutate(Model="FTNIR2")  
cbind(t1, t2[,3], t3[,3]) |> gt::gt()
  tab |> mutate(edf = round(edf, 1)) |> 
  gt::gt() |> 
  gt::fmt_number(columns = 3, decimals = 1) |> 
  gt::fmt_markdown(columns = 1) |> 
  gt::cols_label(par_names = "Parameter name", N="N", edf = "Effective degrees of freedom") |>
  gt::tab_spanner(label = "Effective degrees of freedom", columns = c(1:3)) 
