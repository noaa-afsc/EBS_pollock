...
as well as a run that downweights all survey biomass indices. @tbl-loo_runs shows the
combinations of data types included for this analysis.

Results show that the model is sensitive to the inclusion of the ... data (@tbl-loo_fits)
...
Among these models, when downweighting the BTS data, the model estimates of spawning biomass
changed in trends @fig-loo-ssb .
The model estimates of recruitment were less dramatic in appearance but was consistent
for the run where all of the bottom-trawl survey data were excluded @fig-loo-recruitment. 
The relative fits to the different indices by data component ommission show that 
the model is relatively insensitive to data being excluded (see @fig-loo-bts-index,
@fig-loo-ats-index,  and @fig-loo-avo-index).  The consistency can also be noted
in the goodness-of-fit measures in @tbl-loo_fits. The table shows the goodness-of-fit
in different ways to the components that are dropped successively.


In conclusion, the data sources suggest a good level of consistency driving the trends--that is
there appears to be relatively low levels of conflict. This is likely due to the 
constant natural mortality-at-age and highly precise fishery catch-at-age. That is,
the traditional "VPA" approach of back-calculating the abundance based on the numbers caught
result in a relatively consistent estimate of the population size.  The importance of the survey data can be seen in 
some of the reference calculations where the uncertainty increases
dramatically and the stock-status perception is quite differen @tbl-loo_ref.
...
@fig-synts shows the model results comparing the last year's model some alternative model
platforms. In @fig-synts, comparing the SS3 output with three different configurations on
selectivity constraint shows similarities in the time series. Comparing the SS3 output
with last year's reference model shows that xxx @fig-ss3tsref.
...
but slightly lower inter-annual variability in the SAM run (@fig-sam). The spawning
biomass trend largely overlaps in the uncertainty @fig-sam_ts.
...
(Plot_Rec(rbind(sam_run$ts, pm_run$ts)) + ggthemes::theme_few(base_size = 19))
# all_sel <- rbind(pm_run$sel,r1$sel,c2$sel)#,gp_run$sel)
# ss_sel  <- rbind(r1$sel,r2$sel,r3$sel)#,r4$sel)#,r5$sel)
```
...
lab and the University of Alaska, Fairbanks (@spock). @fig-spock_sel shows the model
results comparing time-varying selectivity whereas @fig-spock_ts shows the model results
time series. The difference in the age-1 recruits is likely due to the fact that the
spatial model currently has constant M-at-age compared to the base model.
