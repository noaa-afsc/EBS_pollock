...
For the model-based index of Bering Sea pollock, we fitted observations of numerical abundance
or biomass per unit area from all sampling locations in the 83-112 bottom trawl survey of the
...
All models were fitted in the sdmTMB R package(https://pbs-assess.github.io/sdmTMB; @Anderson2022). 
The cold pool covariate was computed within the coldpool R package
...
For model-based estimation of age composition, we fitted observations
...
extrapolation grid as implemented for abundance indices, but modeled spatial and
...
(@fig-sdm_age). The differences between the VAST and tinyVAST age composition models are
likely because VAST estimates a separate kappa (the parameter that represents the spatial 
decorrelation rate) for the fist and second linear predictors, whereas tinyVAST estimates 
a single kappa (@fig-sdm_age).
...
(https://github.com/afsc-gap-products/model-based-indices). 
...
#| messages: FALSE
#| label: fig-sdm_age
#| fig.cap: "Model results comparing last year's model with the revised tinyVAST model."
