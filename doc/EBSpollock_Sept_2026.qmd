...
count conversion. The fishery and BTS retain every input year. ATS 2020 remains
in the fitted Rceattle likelihood and full internal OSA calculation. Its
sample size of one is distributed among 15 ages, and every age-specific count
rounds to zero in the `afscOSA` conversion. The common display therefore uses
the other 18 ATS years. Across all 19 ATS years, the Rceattle internal result
contains 266 residuals, has an SDNR of 0.534, and has an expected interval of
0.915--1.085.

Each SDNR is below its expected interval, with the ATS showing the lowest
value (@tbl-model-26-osa). The aggregate fits and residual bubbles show the age,
year, and cohort locations of the remaining patterns
(@fig-model-26-osa). The current Rceattle decomposition also marked 188
extremely small conditional predictions as uninformative, including slight
values below zero. Interpretation therefore emphasizes aggregate fits and
cells supported by appreciable predicted counts.
...
| ATS | 18 | 252 | 0.536 | 0.913--1.087 | 930 | 914 | 680 |
...
Rceattle `Sample_size` field over the displayed years. The rounded count total
and effective sample size come from the same `afscOSA` plotting conversion.
{#tbl-model-26-osa}

![Aggregate age-composition fits, OSA quantile plots, and residual bubbles for the fishery, BTS, and ATS. The fishery and BTS panels retain every input year. The ATS panels retain 18 years because its sample size of one in 2020 produces zero rounded counts in the plotting conversion.](assets/model-26-0/model_26_0_afscosa.png){#fig-model-26-osa fig-alt="Four rows of diagnostic plots for fishery, bottom-trawl survey, and acoustic-trawl survey age compositions. The fishery and bottom-trawl survey panels contain all input years, while the acoustic-trawl survey panels contain 18 years and omit 2020 from the display because its sample size of one produces zero rounded age counts. The first row compares aggregate observed and fitted proportions. The second row shows OSA quantile plots with residual standard deviations below one. The lower rows show OSA and Pearson residual bubbles by age and year." width="100%"}
