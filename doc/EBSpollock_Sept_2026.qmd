...
comments and response plan, Appendix 2 documents observed length-frequency
patterns, and Appendix 3 provides the development framework for an Ecosystem
...
forms, and the two-stage fitting sequence (@tbl-model-26-configuration).
...
contributed only small differences. The Mohn's rho comparison and terminal-year
patterns show the close agreement between the two lag-preserving analyses and
the much larger native-routine revisions
(@tbl-model-26-retro-sensitivity; @fig-model-26-retro-sensitivity). The
sensitivity combines the lag, terminal-selectivity, and fitting-sequence
differences; a future factorial test can apportion their individual
contributions. The lag-preserving 5.23.0 results provide the primary Model 26.0
...
and Alternative 7 moves below it in 2028
(@fig-model-26-projection; @tbl-model-26-projection).
...
With catch fixed at 1,300 thousand t, the displayed $B/B_{35\%}$ values remain
