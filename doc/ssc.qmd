...
tracking labels used to document the transition from the 2024 ADMB model; the
...
flowchart LR
  A["Model 23A<br/>2024 ADMB source"] -->|"direct translation<br/>and checks"| B["Model 23B<br/>translation check"]
...
- **Model 23A** identifies the ADMB source used in 2024. This is the tracking
...
- **Model 23B** identifies the direct RTMB translation used to check that the
  new code reproduces the source model. It is a tracking label, not a second
  public model number.
...
|---|---|
| Use the 2024 base model as the source for an RTMB implementation, with routine data updates. | Use the accepted 2024 ADMB configuration as the scientific source. First check the direct RTMB translation under the 23B tracking label, then use Model 25.0 (RTMB) for the assessment line begun in 2025. List each new data set and each model change separately. Save the model version, run files, diagnostics, projections, and management quantities used in the report. |
