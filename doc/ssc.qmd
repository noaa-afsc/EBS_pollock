...
- The next assessment needs to address the 2024 RTMB starting model, updated
  survey data, 2025 FT-NIRS ages, the comparison of traditional and FT-NIRS
  ages, model names, and the document archive.
...
|---|---|
| Use the 2024 base model implemented in RTMB, with routine data updates. | Use the accepted 2024 RTMB model as the starting point for the next assessment. List each new data set and each model change separately. Give every model a clear name or number. Save the model version, run files, diagnostics, projections, and management quantities used in the report. |
...
| Develop a plan with the BSAI Groundfish Plan Team for moving beyond ADMB. | Compare RTMB, Rceattle, and SPoRC with the same data and a common set of tests. Ask the Plan Team to agree on the tests before selecting the model software for future assessments. Keep the accepted assessment model as the control until a replacement gives stable estimates, sound uncertainty, acceptable retrospective results, and reproducible projections. |
...
| Clearly identify the models proposed for the assessment, including model numbers. | Use the same short model names in the report, presentations, run folders, tables, and archive. For every model, list its purpose, parent model, data set, model changes, software version, and review status. State clearly which model is recommended for management advice. |
