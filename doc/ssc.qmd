...
software name should be shown separately. Under this plan, `23A` and `23B` are
lineage labels used to document the transition from the 2024 ADMB model; the
public model names are **Model 25.0 (RTMB)** for the work begun in 2025 and
**Model 26.0 (Rceattle)** for the development line begun in 2026.

```{mermaid}
%%| label: fig-model-numbering
%%| fig-cap: "Proposed numbering and lineage for EBS pollock assessment models."
%%| fig-alt: "Flowchart beginning with the Model 23A ADMB source used in 2024. One branch passes through the Model 23B translation check to Model 25.0 in RTMB, begun in 2025. A second branch leads to Model 26.0 in Rceattle, begun in 2026. SPoRC supports comparisons with the two development lines but does not yet have an assessment model number."

flowchart LR
  A["Model 23A<br/>2024 ADMB source"] -->|"direct translation<br/>and checks"| B["Model 23B<br/>lineage label"]
...
- **Model 23A** identifies the ADMB source used in 2024. This is the lineage
  label for the previously accepted Model 23.0 configuration.
- **Model 23B** identifies the direct RTMB translation used to check that the
  new code reproduces the source model. It is a lineage label, not a second
  public model number.
...
|---|---|
| Use the 2024 base model as the source for an RTMB implementation, with routine data updates. | Use the accepted 2024 ADMB configuration as the scientific source. First check the direct RTMB translation under the 23B lineage label, then use Model 25.0 (RTMB) for the assessment line begun in 2025. List each new data set and each model change separately. Save the model version, run files, diagnostics, projections, and management quantities used in the report. |
...
| Develop a plan with the BSAI Groundfish Plan Team for moving beyond ADMB. | Use the model lineage and numbering plan above. Compare RTMB, Rceattle, and SPoRC with the same data and a common set of tests. Ask the Plan Team to agree on the tests before selecting the model software for future assessments. Keep the accepted assessment model as the control until a replacement gives stable estimates, sound uncertainty, acceptable retrospective results, and reproducible projections. |
...
| Clearly identify the models proposed for the assessment, including model numbers. | Use Model 25.0 (RTMB) for the line begun in 2025 and Model 26.0 (Rceattle) for the line begun in 2026. Retain 23A and 23B only as lineage labels for the 2024 ADMB source and its direct RTMB translation. Use the same short names in the report, presentations, run folders, tables, and archive. For every model, list its purpose, parent model, data set, model changes, software version, and review status. State clearly which model is recommended for management advice. |
